...
| 特性 | useState（冰箱） | useMemo（计算器） |
|------|------------------|-------------------|
| **核心功能** | 存储和更新状态 | 缓存计算结果 |
| **返回值** | [状态值, setter函数] | 记忆化的值 |
| **能否更新** | ✅ 有setter函数可以更新 | ❌ 没有setter，无法更新 |
| **触发条件** | 调用setState时触发重新渲染 | 依赖项变化时重新计算 |
| **重新执行** | 每次渲染状态值都可用 | 依赖变化时才重新计算 |
| **引用稳定性** | 状态值引用可能变化 | 返回值引用稳定 |

### 🔄 为什么不能互相替代？

```typescript
// ❌ useMemo 无法替代 useState
const [count, setCount] = useState(0); // 可以更新：setCount(1)

// 尝试用 useMemo "替代" - 这是不可能的！
const memoCount = useMemo(() => 0, []); // 无法更新！永远是0
// memoCount = 1; // ❌ 错误：不能赋值给常量
```
